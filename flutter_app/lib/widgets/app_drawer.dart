                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Builder(
                            builder: (context) {
                              final expectedBalance = state.getExpectedNetAssetAtEnd(_drawerCashFlowDate.year, _drawerCashFlowDate.month);
                              return Container(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Text('${_drawerCashFlowDate.month}월 예상 자산', style: GoogleFonts.notoSansKr(fontSize: 15, color: const Color(0xFF0F172A), fontWeight: FontWeight.w600)),
                                    const Spacer(),
                                    Column(
                                  ...
                                  builder: (context) {
                                    ...
                                    Widget buildAccNode(Account acc, {bool isSubItem = false}) {
                                      ...
                                      if (acc.linkedBankAccountId != null) {
                                        final matches = state.accounts.where((a) => a.id == acc.linkedBankAccountId);
                                        if (matches.isNotEmpty) linkedBankName = matches.first.name;
                                      }
                                      ...
                                      if (acc.isBank) {
                                        ...
                                        int bStartM = acc.billingStartMonth ?? -1;
                                        int bStartD = acc.billingStartDay ?? 1;
                                        int bEndM = acc.billingEndMonth ?? -1;
                                        int bEndD = acc.billingEndDay ?? 31;
                                        int pDay = acc.paymentDay ?? 1;
                                        
                                        String formatMonthDay(int mOffset, int day) {
                                          DateTime d = DateTime(now.year, now.month + mOffset, 1);
                                          int actualDay = day == 31 ? DateTime(d.year, d.month + 1, 0).day : day;
                                          return '${d.month.toString().padLeft(2, '0')}/${actualDay.toString().padLeft(2, '0')}';
                                        }
                                        
                                        rightTopText = '${formatMonthDay(bStartM, bStartD)} ~ ${formatMonthDay(bEndM, bEndD)}';
                                        rightBottomText = '매월 ${pDay}일 결제';
                                      } else if (acc.isDebit) {
                                        amount = null;
                                        badgeText = '체크';
                                        subLabel = acc.bank;
      ...
      children: [
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isExpanded ? const Color(0xFFE2E8F0) : const Color(0xFFF8FAFC),
              borderRadius: BorderRadius.circular(12),
            ),
            child: Row(
              children: [
