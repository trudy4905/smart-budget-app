                  ...
                  Padding(
                    ...
                    child: Builder(
                      builder: (context) {
                        ...
                        int totalRecurringExpense = 0;
                        List<Widget> expenseWidgets = [];
                        final expenseRecurringMap = <String, Transaction>{};
                        for (final t in state.transactions) {
                          if (t.isRecurring && t.recurringId != null && t.type == 'expense') {
                            expenseRecurringMap[t.recurringId!] = t;
                          }
                        }
                        final expenseTxs = expenseRecurringMap.values.toList();
                        for (final tx in expenseTxs) {
                          totalRecurringExpense += tx.amount;
                          expenseWidgets.add(
                            Padding(
                              ...
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text('${state.currentDate.month}/${int.tryParse(tx.date.split('-').last) ?? 0}', style: GoogleFonts.notoSansKr(fontSize: 11, color: const Color(0xFF475569))),
                                  Text('${formatNumber(tx.amount)}원', style: GoogleFonts.notoSansKr(fontSize: 11, color: const Color(0xFF0F172A))),
                                ],
                              ),
                            ),
                          );
                        }
                        if (expenseWidgets.length > 2) {
                          expenseWidgets = expenseWidgets.sublist(0, 2);
                        }

                        // Calculate recurring income
                        int totalRecurringIncome = 0;
                        List<Widget> incomeWidgets = [];
                        final incomeRecurringMap = <String, Transaction>{};
                        for (final t in state.transactions) {
                          if (t.isRecurring && t.recurringId != null && t.type == 'income') {
                            incomeRecurringMap[t.recurringId!] = t;
                          }
                        }
                        final incomeTxs = incomeRecurringMap.values.toList();
                        ...
                        if (incomeWidgets.length > 2) {
                          incomeWidgets = incomeWidgets.sublist(0, 2);
                        }
                          ...
                          children: [
                            ...
                            Expanded(
                              child: GestureDetector(
                                onTap: () {
                                  setState(() => _isRecurringExpenseExpanded = true);
                                  SharedPreferences.getInstance().then((prefs) => prefs.setBool('isRecurringExpenseExpanded', true));
                                },
                                child: Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Text('이번 달 고정지출', style: GoogleFonts.notoSansKr(fontSize: 13, fontWeight: FontWeight.w700, color: const Color(0xFF0F172A))),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Container(
                                            ...
                                            child: const Icon(Icons.calendar_today, size: 14, color: Color(0xFFE11D48)),
                                          ),
                                          const SizedBox(width: 8),
                                          Expanded(
                                            child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: expenseWidgets.isNotEmpty 
                                                ? expenseWidgets 
                                                : [Text('내역 없음', style: GoogleFonts.notoSansKr(fontSize: 11, color: const Color(0xFF94A3B8)))],
                                            ),
                                          ),
                                        ],
                                      ),
                                      ...
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Text('총', style: GoogleFonts.notoSansKr(fontSize: 12, color: const Color(0xFF475569))),
                                          Text('${formatNumber(totalRecurringExpense)}원', style: GoogleFonts.notoSansKr(fontSize: 14, fontWeight: FontWeight.w700, color: const Color(0xFFE11D48))),
                                        ],
                                      ),
                                    ],
                                  ),
                            ...
                            Expanded(
                              child: GestureDetector(
                                onTap: () {
                                  setState(() => _isRecurringIncomeExpanded = true);
                                  SharedPreferences.getInstance().then((prefs) => prefs.setBool('isRecurringIncomeExpanded', true));
                                },
                                child: Container(
                                  padding: const EdgeInsets.all(16),
                                  decoration: BoxDecoration(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Text('이번 달 고정수입', style: GoogleFonts.notoSansKr(fontSize: 13, fontWeight: FontWeight.w700, color: const Color(0xFF0F172A))),
                                          const Icon(Icons.chevron_right, size: 16, color: Color(0xFF94A3B8)),
                                        ],
                                      ),
                                      const SizedBox(height: 12),
                                      Row(
                                        ...
                                        children: [
                                          Container(
                                            ...
                                            child: const Icon(Icons.calendar_today, size: 14, color: Color(0xFF059669)),
                                          ),
                                          const SizedBox(width: 8),
                                          Expanded(
                                            child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: incomeWidgets.isNotEmpty
                                                ? incomeWidgets
                                                : [Text('내역 없음', style: GoogleFonts.notoSansKr(fontSize: 11, color: const Color(0xFF94A3B8)))],
                                            ),
                                          ),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Text('총', style: GoogleFonts.notoSansKr(fontSize: 12, color: const Color(0xFF475569))),
                                          Text('${formatNumber(totalRecurringIncome)}원', style: GoogleFonts.notoSansKr(fontSize: 14, fontWeight: FontWeight.w700, color: const Color(0xFF059669))),
                                        ],
                                      ),
                                    ],
                                  ),
